...
const QueryMetricsInputSchema = z
    ...
    metric_query: z
        .string()
        .min(1)
        .describe('Datadog metrics query string, e.g. "avg:system.cpu.user{service:api}", "sum:nginx.requests{env:prod} by {host}"'),
    from_time: z
        .number()
        .int()
        .optional()
        .describe("Unix epoch start time in seconds. If omitted, defaults to from_minutes_ago minutes before now."),
    to_time: z
        .number()
        .int()
        .optional()
    ...
    from_minutes_ago: z
        ...
        .max(43200)
        .default(60)
        .describe("How many minutes back to query when from_time is not supplied (default: 60, max: 43200 = 30 days)"),
    max_points_per_series: z
        ...
        .max(500)
        .default(100)
        .describe("Maximum number of data points to include per series (default: 100). The most recent points are kept."),
    response_format: z
...
// Helpers
// ---------------------------------------------------------------------------
function formatMetricsMarkdown(result, query) {
    const lines = [
    ...
    for (const series of result.series) {
        ...
        if (series.scope)
            lines.push(`- **Scope**: \`${series.scope}\``);
        if (series.unit)
            lines.push(`- **Unit**: ${series.unit}`);
...
// Tool registration
// ---------------------------------------------------------------------------
export function registerQueryMetrics(server) {
    server.registerTool("datadog_query_metrics", {
...
Error handling:
  ...
  - Returns "No metric data found" if the query yields no series`,
        ...
        try {
            ...
            const raw = await ddGet("/api/v1/query", {
                from: fromTime,
                to: toTime,
            ...
            const series = rawSeries.map((s) => {
                ...
                const unit = Array.isArray(s.unit) && s.unit.length > 0
                    ? (s.unit[0]?.name ?? null)
                    : null;
                return {
            ...
            if (text.length > CHARACTER_LIMIT) {
                ...
                text = JSON.stringify({
                    ...
                    truncated: true,
                    truncation_message: "Data points per series were trimmed to 10. Use max_points_per_series to control output size.",
                }, null, 2);
            }
            return {
