...
const ListMonitorsInputSchema = z
    ...
    name_filter: z
        .string()
        .optional()
        .describe("Filter monitors whose name contains this string (case-insensitive, optional)"),
    tags: z
        .string()
        .optional()
        .describe('Comma-separated list of tags to filter monitors by, e.g. "env:prod,team:backend" (optional)'),
    monitor_status: z
        .string()
        .optional()
        .describe('Filter by current monitor status. Accepted values: "Alert", "Warn", "No Data", "OK", "Ignored", "Skipped" (optional, case-insensitive)'),
    limit: z
        .number()
        .int()
        .min(1)
...
// Helpers
// ---------------------------------------------------------------------------
function formatMonitorsMarkdown(result) {
    const lines = [
        "# Datadog Monitors",
        "",
    ...
    for (const m of result.monitors) {
        ...
        const statusEmoji = status === "Alert" ? "🔴" : status === "Warn" ? "🟡" : status === "OK" ? "🟢" : "⚪";
        ...
        if (m.modified)
            lines.push(`- **Last modified**: ${m.modified}`);
        lines.push("");
    }
...
// Tool registration
// ---------------------------------------------------------------------------
export function registerListMonitors(server) {
    server.registerTool("datadog_list_monitors", {
  ...
  - Returns "No monitors found" if no monitors match the filters`,
        ...
        try {
            ...
            if (params.name_filter)
                queryParams["name"] = params.name_filter;
            if (params.tags)
                queryParams["monitor_tags"] = params.tags;
            const raw = await ddGet("/api/v1/monitor", queryParams);
            ...
            if (params.monitor_status) {
                ...
                monitors = monitors.filter((m) => (m.overall_state ?? "").toLowerCase().includes(filterLower));
            }
