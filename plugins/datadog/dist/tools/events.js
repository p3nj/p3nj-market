...
const ListEventsInputSchema = z
    ...
    from_time: z
        ...
        .int()
        .optional()
        .describe("Unix epoch start time in seconds. If omitted, defaults to from_hours_ago hours before now."),
    to_time: z
        .number()
        .int()
        .optional()
    ...
    from_hours_ago: z
        ...
        .max(8760)
        .default(24)
        .describe("How many hours back to look when from_time is not provided (default: 24, max: 8760 = 1 year)"),
    priority: z
        .enum(["normal", "low"])
        .optional()
    ...
    tags: z
        .string()
        .optional()
        .describe('Comma-separated tags to filter events by, e.g. "env:prod,service:api" (optional)'),
    limit: z
        .number()
        .int()
        .min(1)
...
// Helpers
// ---------------------------------------------------------------------------
function formatEventsMarkdown(result) {
    const lines = [
        "# Datadog Events",
        "",
    ...
    for (const e of result.events) {
        const ts = e.date_happened
            ...
            : "unknown time";
        const alertEmoji = e.alert_type === "error"
            ? "🔴"
        ...
        lines.push(`- **Time**: ${ts}`);
        if (e.alert_type)
            lines.push(`- **Alert type**: ${e.alert_type}`);
        if (e.priority)
            lines.push(`- **Priority**: ${e.priority}`);
        if (e.host)
            lines.push(`- **Host**: \`${e.host}\``);
        if (e.text)
            lines.push(`- **Details**: ${e.text}`);
...
export function registerListEvents(server) {
    server.registerTool("datadog_list_events", {
...
Error handling:
  ...
  - Returns "No events found" if no events match the time range and filters`,
        ...
        try {
            ...
            const raw = await ddGet("/api/v1/events", queryParams);
            ...
            if (text.length > CHARACTER_LIMIT) {
                const truncated = {
                    ...
                    truncated: true,
                    truncation_message: "Response truncated. Use a smaller limit or narrow the time range.",
                };
