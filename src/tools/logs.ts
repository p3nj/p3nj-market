...
import { ResponseFormat, type LogEntry, type LogsResult } from "../types.js";
...
const QueryLogsInputSchema = z
  .object({
    query: z
      ...
      .min(1)
      .describe(
        'Datadog log search query. Supports Datadog query syntax, e.g. "service:api status:error", "env:prod @http.status_code:5*"'
      ),
    from_time: z
      .string()
      .default("now-1h")
      .describe(
        'Start of the time range. Relative values like "now-1h", "now-24h" or ISO-8601 timestamps are accepted (default: "now-1h")'
      ),
    to_time: z
      .string()
      .default("now")
    ...
    cursor: z
      ...
      .optional()
      .describe(
        "Pagination cursor returned by a previous call. Pass this to retrieve the next page of results."
      ),
    response_format: z
...
// ---------------------------------------------------------------------------
// Datadog API response shape (v2 logs search)
// ---------------------------------------------------------------------------
...
function formatLogsMarkdown(result: LogsResult, query: string): string {
  const lines: string[] = [
    `# Datadog Logs: \`${query}\``,
    "",
  ...
  for (const entry of result.logs) {
    ...
    if (entry.severity) lines.push(`- **Severity**: ${entry.severity}${entry.severityNumber != null ? ` (${entry.severityNumber})` : ""}`);
    if (entry.logType) lines.push(`- **Log Type**: ${entry.logType}`);
    if (entry.integration) lines.push(`- **Integration**: ${entry.integration}`);
    if (entry.instance) lines.push(`- **Instance**: ${entry.instance}`);
    if (entry.flow) lines.push(`- **Flow**: ${entry.flow}`);
    ...
    if (entry.retryAttempt != null && entry.retryAttempt > 0) lines.push(`- **Retry Attempt**: ${entry.retryAttempt}`);
    if (entry.isTestExecution) lines.push(`- **Test Execution**: yes`);

    // IDs (collapsed for readability)
    const ids: string[] = [];
    if (entry.executionId) ids.push(`exec=\`${entry.executionId}\``);
    if (entry.instanceId) ids.push(`instance=\`${entry.instanceId}\``);
    if (entry.integrationId) ids.push(`integration=\`${entry.integrationId}\``);
    if (entry.flowId) ids.push(`flow=\`${entry.flowId}\``);
    if (entry.flowConfigId) ids.push(`flowConfig=\`${entry.flowConfigId}\``);
...
export function registerQueryLogs(server: McpServer): void {
  server.registerTool(
    "datadog_query_logs",
    {
  ...
  - Returns "No logs found" if the query returns zero results`,
    ...
    async (params: QueryLogsInput) => {
      try {
        ...
        const raw = await ddPost<DDLogsResponse>(
        ...
        const logs: LogEntry[] = entries.map((e) => {
          ...
          return {
            ...
            severity: (inner.severity as string) ?? null,
            severityNumber: typeof inner.severityNumber === "number" ? inner.severityNumber : null,
            logType: (inner.logType as string) ?? null,
            instance: (inner.instance as string) ?? null,
            instanceId: (inner.instanceId as string) ?? null,
            integration: (inner.integration as string) ?? null,
            integrationId: (inner.integrationId as string) ?? null,
            flow: (inner.flow as string) ?? null,
            flowId: (inner.flowId as string) ?? null,
            flowConfigId: (inner.flowConfigId as string) ?? null,
            executionId: (inner.executionId as string) ?? null,
